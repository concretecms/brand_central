    ...
    render () {
        const tags = this.props.tags.map(tag => 
            <span className="label label-default" key={tag.id}>
                {tag.name}  
                <span onClick={()=>this.props.removeTag(tag.id,this.props.asset)} className="remove-tag-btn"> <i className="fas fa-times"></i> </span>
            </span>)

        return (
            <section>
                <span className="add-tag-btn" onClick={()=>this.props.openModal(this.props.asset, this.props.file)}>Add Tag <i className="fas fa-plus"></i></span>
